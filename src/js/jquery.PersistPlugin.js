...
// #################Above code was working but due to some ipfs and wallet isssue updating#####
...
//         $.getScript("https://unpkg.com/ipfs-http-client/dist/index.min.js", function (data, textStatus, xhr) {
...
//         });
//         var parent = this;
//         ipfs.add({ path: '/', content: new TextEncoder().encode(JSON.stringify(obj)) }, function (err, files) {
//             if (err) {
//                 console.error(err);
//                 return;
//             }
...
(function ($) {
    ...
    $.loadIPFS = function (hash) {
        $.getJSON("http://localhost:8080/ipfs/" + hash, function (data) {
            data["ipfs-hash"] = hash;
            $.each(data, function (i, v) {
                if (i != "ipfs-hash") {
                    $("#" + i).val(v);
